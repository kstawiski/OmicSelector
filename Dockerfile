...
RUN apt-get update --fix-missing && \
    apt-get install -y apt-utils sshfs libxml2-dev cifs-utils libffi-dev libssl-dev wget bzip2 ca-certificates build-essential cmake git unzip pkg-config libopenblas-dev liblapack-dev libhdf5-serial-dev libglib2.0-0 libfontconfig1-dev libxext6 libsm6 libxrender1 gfortran-7 gcc-7 && apt-get clean && \
    conda update --all && conda install --channel "conda-forge" --channel "anaconda" --channel "r" tensorflow-mkl r-biocmanager gxx_linux-64 gcc_linux-64 keras jupyter jupytext numpy fontconfig pandas r=4.0.2 r-rlang r-devtools r-pkgdown r-brio r-magick r-rgl r-rjava r-mnormt r-purrrogress r-xml libxml2 libmagic pandoc r-rjava opencv pkgconfig gfortran_linux-64 && echo "options(repos=structure(c(CRAN='http://cran.r-project.org')))" >> ~/.Rprofile
...
RUN Rscript /keras.R && Rscript -e "remotes::install_github('kstawiski/OmicSelector', force = T); library(OmicSelector);"
