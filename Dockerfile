...
RUN apt-get update --fix-missing && \
    ...
    conda install mamba -c conda-forge && mamba install --channel "conda-forge" --channel "anaconda" --channel "r" r-base>4.0 keras jupyter jupytext numpy pandas libxml2 pandoc r-rjava r-magick libglu libx11-devel-cos6-x86_64 libgit2 xorg-libx11 mesa-libgl-cos6-x86_64 r-devtools r-rgl r-cairo opencv r-gert mesa && echo "options(repos=structure(c(CRAN='http://cran.r-project.org')))" >> ~/.Rprofile
...
# RStudio server:
RUN apt-get install -y libclang-dev && wget https://www.rstudio.org/download/latest/stable/server/bionic/rstudio-server-latest-amd64.deb && dpkg -i rstudio-server-latest-amd64.deb && apt -f -y install && cd / && git clone https://github.com/grst/rstudio-server-conda.git && chmod 777 -R /opt/conda && rm rstudio-server-latest-amd64.deb
COPY docker/start_rstudio_server.sh /start_rstudio_server.sh
COPY docker/rsession.sh /rsession.sh

# MXNET:
RUN pip install --upgrade cmake && cd / && git clone --recursive https://github.com/apache/incubator-mxnet.git -b v1.x && cd /incubator-mxnet && mkdir build && cd build && cmake -DUSE_CUDA=OFF -DUSE_MKL_IF_AVAILABLE=ON -DUSE_MKLDNN=OFF -DUSE_OPENMP=ON -DUSE_OPENCV=ON .. && make -j $(nproc) USE_OPENCV=1 USE_BLAS=openblas && make install && cp -a . .. && cp -a . ../lib && cd /incubator-mxnet/ && make -f R-package/Makefile rpkg
