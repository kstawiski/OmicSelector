...
# RStudio server:
RUN apt-get install -y libclang-dev && wget https://www.rstudio.org/download/latest/stable/server/bionic/rstudio-server-latest-amd64.deb && dpkg -i rstudio-server-latest-amd64.deb && apt -f -y install && cd / && git clone https://github.com/grst/rstudio-server-conda.git
