...
RUN apt-get update --fix-missing && \
    ...
    conda update --all && conda install mamba -c conda-forge && mamba install --channel "conda-forge" --channel "anaconda" --channel "r" tensorflow-gpu keras jupyter jupytext numpy pandas opencv && echo "options(repos=structure(c(CRAN='http://cran.r-project.org')))" >> ~/.Rprofile
