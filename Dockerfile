...
RUN apt-get update --fix-missing && \
    ...
    conda install mamba -c conda-forge && mamba install --channel "conda-forge" --channel "anaconda" --channel "r" r-base>4.0 keras jupyter jupytext numpy pandas libxml2 pandoc r-rjava r-magick libglu libx11-devel-cos6-x86_64 libgit2 xorg-libx11 mesa-libgl-cos6-x86_64 r-devtools r-rgl r-cairo opencv && echo "options(repos=structure(c(CRAN='http://cran.r-project.org')))" >> ~/.Rprofile
...
# RStudio server:
RUN wget https://www.rstudio.org/download/latest/stable/server/bionic/rstudio-server-latest-amd64.deb && dpkg -i rstudio-server-latest-amd64.deb && apt -f -y install && systemctl disable rstudio-server.service && systemctl stop rstudio-server.service && cd / && git clone https://github.com/grst/rstudio-server-conda.git
