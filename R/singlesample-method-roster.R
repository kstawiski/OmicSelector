...
#' (`inst/extdata/singlesample_method_manifest.csv`) so the R package and the
...
#' @seealso [singlesample_method_bank()], [singlesample_score_call()],
#'   [singlesample_assert_row_equivariant()]
#' @export
singlesample_method_roster <- function() {
  path <- system.file("extdata", "singlesample_method_manifest.csv",
                      package = "OmicSelector")
  if (!nzchar(path) || !file.exists(path)) {
    stop("singlesample_method_manifest.csv not found in package extdata.",
         call. = FALSE)
  }
  ...
  if (length(miss) > 0L) {
    stop("singlesample method manifest is missing columns: ",
         paste(miss, collapse = ", "), call. = FALSE)
  }
  m <- m[, req_cols, drop = FALSE]
  .singlesample_validate_roster(m)
...
#' fails the gate.
#'
#' @return Character vector of 74 method identifiers.
#' @keywords internal
.singlesample_frozen_roster_ids <- function() {
  ...
  transfer_existing <- c(                                # 17 existing transfer disc
    ...
    "dann", "cvae", "icp")
  within_new <- c(                                       # 37 new within disc
    "reo-ktsp", "reo-pairratio", "reo-singscore", "reo-ucell",
    "bal-selbal", "ws-balance-ilr", "lrt-bw", "lrt-nbkde", "lrt-copula",
    "lrt-tcopula", "conf-mondrian",
...
.singlesample_roster_enums <- list(
...
.singlesample_validate_roster <- function(m) {
  if (anyNA(m$method_id) || any(!nzchar(m$method_id))) {
    stop("singlesample roster has missing method_id values.", call. = FALSE)
  }
  if (any(duplicated(m$method_id))) {
    stop("singlesample roster has duplicate method_id values: ",
         paste(unique(m$method_id[duplicated(m$method_id)]), collapse = ", "),
         call. = FALSE)
  }
  for (col in names(.singlesample_roster_enums)) {
    allowed <- .singlesample_roster_enums[[col]]
    bad <- setdiff(unique(m[[col]]), allowed)
    if (length(bad) > 0L) {
      stop(sprintf("singlesample roster column '%s' has disallowed values: %s",
...
# a shim via singlesample_register_score_adapter() in their own per-method gate.
#
...
.singlesample_score_adapter_registry <- new.env(parent = emptyenv())
...
#' @seealso [singlesample_score_call()]
#' @export
singlesample_register_score_adapter <- function(method_id, fn) {
  ...
  assign(method_id, fn, envir = .singlesample_score_adapter_registry)
  invisible(TRUE)
}
...
#' [singlesample_register_score_adapter()] (their argument order predates the canonical
...
#' @seealso [singlesample_method_roster()], [singlesample_assert_row_equivariant()]
#' @export
singlesample_score_call <- function(method_id, model, X, meta = NULL,
  ...
  X <- .singlesample_as_score_matrix(X)
  if (!is.null(meta)) {
  ...
  if (exists(method_id, envir = .singlesample_score_adapter_registry,
             inherits = FALSE)) {
    adapter <- get(method_id, envir = .singlesample_score_adapter_registry,
                   inherits = FALSE)
    ...
    stop(sprintf(paste0("method '%s' is a present/legacy-signature scorer: ",
                        ...
                        "singlesample_register_score_adapter('%s', ",
                        "function(model, X, meta) ...) before scoring it ",
                        "through singlesample_score_call()."),
                 method_id, method_id), call. = FALSE)
  } else {
    adapter <- .singlesample_canonical_adapter(method_id, row$score_fn[[1]])
  }
  out <- adapter(model, X, meta)
  .singlesample_validate_score_output(out, nrow(X), method_id)
}

.singlesample_canonical_adapter <- function(method_id, score_fn_name) {
  if (is.na(score_fn_name) || !nzchar(score_fn_name)) {
    stop(sprintf("method '%s' has no score function declared.", method_id),
         call. = FALSE)
  }
  fn <- .singlesample_resolve_fn(score_fn_name)
  if (is.null(fn)) {
    stop(sprintf(paste0("method '%s' is not yet implemented: score function ",
                        ...
                        "adapter via singlesample_register_score_adapter()."),
...
.singlesample_resolve_fn <- function(name) {
...
.singlesample_as_score_matrix <- function(X) {
...
.singlesample_validate_score_output <- function(out, n, method_id) {
  d <- dim(out)
...
#'   case.
#' @seealso [singlesample_is_row_equivariant()]
#' @export
singlesample_assert_row_equivariant <- function(score_fun, model, X, meta = NULL,
                                          n_perm = 4L, tol = 1e-8,
                                          n_singletons = 4L,
  ...
  if (is.null(model_digest)) model_digest <- .singlesample_default_model_digest
  if (!is.function(model_digest)) stop("model_digest must be a function.", call. = FALSE)

  old_seed <- .singlesample_get_seed()
  on.exit(.singlesample_restore_seed(old_seed), add = TRUE)
  set.seed(seed)

  before <- model_digest(model)
  base <- .singlesample_num_scores(score_fun(model, X, meta), n, "full batch")

  # (c) row-order permutations
  for (i in seq_len(n_perm)) {
    .singlesample_assert_batch(score_fun, model, X, meta, base,
...
#'
#' Convenience wrapper around [singlesample_assert_row_equivariant()] returning a
...
#'
#' @inheritParams singlesample_assert_row_equivariant
#' @return `TRUE` if row-equivariant, `FALSE` otherwise.
#' @seealso [singlesample_assert_row_equivariant()]
#' @export
singlesample_is_row_equivariant <- function(score_fun, model, X, meta = NULL,
                                      n_perm = 4L, tol = 1e-8,
                                      n_singletons = 4L, model_digest = NULL,
                                      seed = 1L) {
  ok <- tryCatch({
    singlesample_assert_row_equivariant(score_fun, model, X, meta, n_perm, tol,
...
.singlesample_assert_batch <- function(score_fun, model, X, meta, base, src, extreme,
                                 tol, where) {
  if (length(extreme) == 1L) extreme <- rep(extreme, length(src))
  Xb <- X[src, , drop = FALSE]
  if (any(extreme)) Xb[extreme, ] <- Xb[extreme, , drop = FALSE] * 1e6
  mb <- .singlesample_subset_meta(meta, src)
  sb <- .singlesample_num_scores(score_fun(model, Xb, mb), length(src), where)
  anchored <- !extreme
  .singlesample_check_close(sb[anchored], base[src[anchored]], tol, where)
}

.singlesample_default_model_digest <- function(model) {
  ...
  if (.singlesample_contains_externalptr(model)) {
...
.singlesample_contains_externalptr <- function(x, seen = list(), depth = 0L) {
  ...
  if (tx == "environment") {
    ...
    for (nm in ls(x, all.names = TRUE)) {
      ...
      if (.singlesample_contains_externalptr(val, seen, depth + 1L)) return(TRUE)
    }
    return(FALSE)
  }
  if (tx == "closure") {
    return(.singlesample_contains_externalptr(environment(x), seen, depth + 1L))
  }
  if (is.list(x) || tx == "pairlist") {
    for (el in x) {
      if (.singlesample_contains_externalptr(el, seen, depth + 1L)) return(TRUE)
    }
  }
  at <- attributes(x)
  if (!is.null(at)) {
    for (a in at) {
      if (.singlesample_contains_externalptr(a, seen, depth + 1L)) return(TRUE)
    }
  }
  FALSE
}

.singlesample_num_scores <- function(out, n, where) {
  d <- dim(out)
  if (!is.null(d) && !(length(d) == 2L && d[2] == 1L)) {
...
.singlesample_check_close <- function(a, b, tol, where) {
  d <- max(abs(a - b))
...
.singlesample_subset_meta <- function(meta, idx) {
  if (is.null(meta)) return(NULL)
  meta[idx, , drop = FALSE]
}

.singlesample_get_seed <- function() {
...
.singlesample_restore_seed <- function(old) {
  if (is.null(old)) {
    if (exists(".Random.seed", envir = globalenv(), inherits = FALSE)) {
