...
#'
#' This is the legacy implementation preserved for backward compatibility.
#' Users should call `OmicSelector_OmicSelector()` which routes through the
#' deprecation wrapper, or preferably migrate to `OmicPipeline`.
#'
...
#'
#' **WARNING: This function has known data leakage issues:**
#' - SMOTE is applied BEFORE cross-validation splits
#' - Feature selection runs on full training set before fold isolation
...
#' @param max_iterations Maximum number of iterations in selected methods. Setting this too high may results in very long computing time.
...
#' @param register_parallel Where to use parallel processing to speed up computing time. Setting it to FALSE may aid in debugging.
...
#' @param conda_path Path to "conda" binary used for executing python scripts.
#' @param debug Gives additional debug information (saves .rdata after feature selection is completed, prints formulas to log)
#' @param timeout_sec Timeout after the method is terminated if not finished. It may be useful to keep the long methods limited, not to wait eternity for the results.
#' @param type Parameter 'mode' forwarded to OmicSelector_differential_expression_ttest which is essential in many feature selection methods. Note that if 'var_type.txt' file exists in the working directory it is superior to the value set directly in function - as designed for GUI.
#'
#' @return The list of selected formulas.
#'
#' @seealso [OmicPipeline], [BenchmarkService], [OmicSelector_OmicSelector_wrapper]
#'
#' @keywords internal
#' @noRd
OmicSelector_OmicSelector_legacy = function(wd = getwd(), m = c(1:70),
