...
#' Methods provided:
#' \itemize{
#'   \item \code{\link{singlesample_matched_null_benchmark}}: main benchmark function.
#'   \item \code{\link{singlesample_hanley_mcneil_auc_ci}}: closed-form
#'     Hanley-McNeil 1982 95\% AUC confidence interval helper.
#'   \item \code{\link{singlesample_bh_fdr_correct_matched_null}}: BH-FDR within a
#'     modality family.
#'   \item \code{\link{singlesample_holm_correct_familywise}}: Holm correction for
#'     family-wise method × claim-state contrasts.
#'   \item \code{\link{singlesample_bh_fdr_correct_blocked}}: block-aware two-stage
...
# ----------------------------------------------------------------------------
# .singlesample_match_panel_strata — internal helper
# ----------------------------------------------------------------------------

#' @noRd
.singlesample_match_panel_strata <- function(panel,
                                        feature_pool,
                                        detection_rate,
                                        mean_abundance,
                                        n_strata = 4L) {
...
# ----------------------------------------------------------------------------
# .singlesample_draw_matched_panel — internal helper
# ----------------------------------------------------------------------------

#' @noRd
.singlesample_draw_matched_panel <- function(strata_info, exclude = character(0)) {
...
singlesample_matched_null_benchmark <- function(X, y, panel, scoring_fn,
  ...
  if (!requireNamespace("pROC", quietly = TRUE)) {
    stop("singlesample_matched_null_benchmark requires pROC for AUC computation.")
  }
  ...
  if (length(excluded_in_pool) > 0L) {
    warning("singlesample_matched_null_benchmark: ", length(excluded_in_pool),
  ...
  strata_info <- .singlesample_match_panel_strata(
  ...
  auc_ci <- .singlesample_auc_ci(y, score_obs, auc_obs, auc_ci_method)
  ...
  fallback_count <- 0L
  for (k in seq_len(K)) {
    p_k <- .singlesample_draw_matched_panel(strata_info, exclude = panel)
    if (length(p_k) == 0L) {
...
# ----------------------------------------------------------------------------
# .singlesample_auc_ci -- shared internal AUC CI helper
# ----------------------------------------------------------------------------

#' @noRd
.singlesample_auc_ci <- function(y, scores, auc_obs,
  ...
  ci <- singlesample_hanley_mcneil_auc_ci(auc_obs, n_pos, n_neg)
...
#'
#' @export
singlesample_hanley_mcneil_auc_ci <- function(auc, n_pos, n_neg,
                                         conf_level = 0.95) {
...
#'
#' @export
singlesample_bh_fdr_correct_matched_null <- function(results) {
...
#'
#' @export
singlesample_holm_correct_familywise <- function(p_values) {
...
#'
#' @export
singlesample_bh_fdr_correct_blocked <- function(results, block_id, rho = 0.25) {
...
#' @noRd
.singlesample_null_coalesce <- function(a, b) if (is.null(a)) b else a

#' Stratified k-folds (ungrouped). Returns list of k test-index vectors.
#' @noRd
.singlesample_stratified_folds <- function(y, k = 5L, seed = 42L) {
  set.seed(seed)
...
#' @noRd
.singlesample_grouped_folds <- function(y, group_id, k = 5L, seed = 42L) {
  set.seed(seed)
...
#' @noRd
.singlesample_select_panel_training_fold <- function(X_train, y_train, panel_size = 20L) {
  if (!requireNamespace("pROC", quietly = TRUE)) {
    stop(".singlesample_select_panel_training_fold requires pROC.")
  }
  n_feat <- ncol(X_train)
...
#'
#' @description
#' Nested cross-validation variant of \code{\link{singlesample_matched_null_benchmark}}.
...
#'
#' @export
singlesample_matched_null_benchmark_cv <- function(
    X,
    y,
    panel_size               = 20L,
    scoring_fn               = NULL,
  ...
  if (!requireNamespace("pROC", quietly = TRUE)) {
    stop("singlesample_matched_null_benchmark_cv requires pROC.")
  }
  auc_ci_method <- match.arg(auc_ci_method)
  stopifnot(is.matrix(X) || is.data.frame(X))
  X <- as.matrix(X)
  ...
  if (!is.finite(seed) || is.na(seed) || seed < 0L || seed > 21474L) {
    stop("singlesample_matched_null_benchmark_cv: seed must be an integer from 0 to 21474.")
  }
  ...
  folds <- if (has_groups)
    .singlesample_grouped_folds(y, group_id, k = outer_k, seed = seed)
  else
    .singlesample_stratified_folds(y, k = outer_k, seed = seed)
  ...
  if (has_groups) {
    for (fi in seq_along(folds)) {
      ...
      if (viol > 0L)
        stop(sprintf(
          "singlesample_matched_null_benchmark_cv: %d group(s) split across train/test in fold %d.",
          viol, fi))
    }
  }
  ...
  for (fi in seq_len(outer_k)) {
    ...
    panel_f <- tryCatch(
      .singlesample_select_panel_training_fold(X_train_pool, y_train, panel_size),
      error = function(e) NULL)
    ...
    .run_null_draw <- function(k) {
      ...
      p_k <- .singlesample_draw_matched_panel_with_tier(strata_info, exclude = panel_f)
    ...
    if (any(vapply(null_results, inherits, logical(1L), "try-error"))) {
      stop(sprintf("singlesample_matched_null_benchmark_cv: null draw failed in fold %d.", fi))
    }
  ...
  skip_summary <- paste(vapply(fold_audit, function(fa)
    .singlesample_null_coalesce(fa$skip_reason, "ok"), character(1L)), collapse = "; ")
  ...
  fold_fallback_frac <- vapply(valid_fi, function(fi) {
    fb <- .singlesample_null_coalesce(fold_audit[[fi]]$fold_fallback_count, NA_integer_)
  ...
  total_fallback <- sum(vapply(fold_audit[valid_fi], function(fa)
    .singlesample_null_coalesce(fa$fold_fallback_count, 0L), integer(1L)))
  ...
  if (auc_ci_method == "delong") {
    auc_ci <- .singlesample_auc_ci(pooled_y, pooled_scores, auc_obs_cv, "delong")
  } else if (auc_ci_method == "hanley_mcneil") {
    auc_ci <- .singlesample_auc_ci(pooled_y, pooled_scores, auc_obs_cv, "hanley_mcneil")
  } else {
