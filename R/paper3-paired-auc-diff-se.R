...
#' difference, matching Paper 3's `auc_obs_cv = mean(auc_obs_folds)` estimand.
#'
...
#'
#' @export
paper3_paired_auc_diff_se <- function(y, score_method, score_baseline,
                                      fold = NULL,
                                      orient = c("median", "auc")) {
  ...
  rows <- lapply(fold_levels, function(f) {
    ...
    res <- .paper3_delong_var_diff_fold(
...
.paper3_delong_placements <- function(y, score, orient = c("median", "auc")) {
  orient <- match.arg(orient)
...
.paper3_delong_var_diff_fold <- function(y, score_method, score_baseline,
                                         orient = c("median", "auc")) {
  orient <- match.arg(orient)
  ...
  pm <- .paper3_delong_placements(y, score_method, orient = orient)
  pb <- .paper3_delong_placements(y, score_baseline, orient = orient)
