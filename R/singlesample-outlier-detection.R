...
#' @noRd
.singlesample_compose_transform <- function(x, transform = "rclr", pseudocount = 0.5) {
  x <- x + pseudocount
  log_x <- log(x)
  if (transform == "rclr") {
    ...
    D <- ncol(log_x)
    H <- .singlesample_helmert_basis(D)
    return(log_x %*% H)
  }
...
#' @noRd
.singlesample_helmert_basis <- function(D) {
  H <- matrix(0, D, D - 1L)
...
#' @noRd
.singlesample_itree_split <- function(x, max_depth = 8L, depth = 0L) {
  n <- nrow(x)
  ...
  list(
    ...
    feat  = feat,
    split = split_val,
    left  = .singlesample_itree_split(left,  max_depth, depth + 1L),
    right = .singlesample_itree_split(right, max_depth, depth + 1L)
  )
}

#' @noRd
.singlesample_itree_path_length <- function(tree, x) {
  if (tree$type == "leaf") return(tree$depth + .singlesample_c_factor(tree$n))
  if (x[tree$feat] < tree$split) .singlesample_itree_path_length(tree$left,  x)
  else                            .singlesample_itree_path_length(tree$right, x)
}

#' @noRd
.singlesample_c_factor <- function(n) {
  if (n <= 1L) return(0)
  2 * (log(n - 1L) + 0.5772156649) - 2 * (n - 1L) / n
}

#' @noRd
.singlesample_rclr <- function(x, pseudocount = 0.5) {
  x <- x + pseudocount
  log_x <- log(x)
  centering <- rowMeans(log_x, na.rm = TRUE)
...
fit_isolation_forest_logratio <- function(x_train,
                                           ...
                                           seed = 42L) {
  if (!is.matrix(x_train)) x_train <- as.matrix(x_train)
  z_train <- .singlesample_rclr(x_train, pseudocount = pseudocount)
  n <- nrow(z_train)
  set.seed(seed)
  if (is.null(max_depth)) max_depth <- ceiling(log2(min(sample_size, n)))
  c_n <- .singlesample_c_factor(min(sample_size, n))

  forest <- vector("list", n_trees)
  for (t in seq_len(n_trees)) {
    idx <- sample(n, min(sample_size, n), replace = FALSE)
    forest[[t]] <- .singlesample_itree_split(z_train[idx, , drop = FALSE],
                                        max_depth = max_depth)
  }

  train_scores <- vapply(seq_len(n), function(i) {
    paths <- vapply(forest,
                    function(tr) .singlesample_itree_path_length(tr, z_train[i, ]),
                    numeric(1L))
    2 ^ (-mean(paths) / c_n)
  }, numeric(1L))
...
apply_isolation_forest_logratio <- function(fit, x_test, fpr_target = 0.05) {
  ...
  x_test <- x_test[, fit$feature_names, drop = FALSE]
  z_test <- .singlesample_rclr(x_test, pseudocount = fit$pseudocount)

  test_scores <- vapply(seq_len(nrow(z_test)), function(i) {
    paths <- vapply(fit$forest,
                    function(tr) .singlesample_itree_path_length(tr, z_test[i, ]),
                    numeric(1L))
