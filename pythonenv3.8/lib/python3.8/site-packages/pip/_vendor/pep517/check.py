"""Check a project and backend by attempting to build using PEP 517 hooOmicSelector_
"""
import argparse
import logging
import os
...
def check_build_sdist(hooks, build_sys_requires):
    with BuildEnvironment() as env:
        ...
        try:
            reqs = hooOmicSelector_get_requires_for_build_sdist({})
        ...
        try:
            try:
                filename = hooOmicSelector_build_sdist(td, {})
...
def check_build_wheel(hooks, build_sys_requires):
    with BuildEnvironment() as env:
        ...
        try:
            reqs = hooOmicSelector_get_requires_for_build_wheel({})
            log.info('Got build requires: %s', reqs)
        except Exception:
            log.error('Failure in get_requires_for_build_sdist', exc_info=True)
            return False
        ...
        try:
            try:
                filename = hooOmicSelector_build_wheel(td, {})
