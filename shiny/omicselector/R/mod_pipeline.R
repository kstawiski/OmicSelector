  ...
  tagList(
    ...
    layout_column_wrap(
      ...
      card(
        ...
        card_body(
          selectInput(ns("filter_method"), "Selection Method",
                      choices = c(
                        "Univariate" = "anova",
      ...
      card(
        ...
        card_body(
          selectInput(ns("model"), "Algorithm",
                      choices = c(
          ...
          accordion(
            ...
            accordion_panel(
              ...
              conditionalPanel(
                condition = sprintf("input['%s'] == 'mlp' || input['%s'] == 'tabtransformer'",
                                    ns("model"), ns("model")),
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$pipeline_summary <- renderText({
      ...
      lines <- c(
        ...
        sprintf("Feature Selection: %s (top %d)", input$filter_method, input$n_features),
        sprintf("Model: %s", input$model)
      )
    ...
    observeEvent(input$create_pipeline, {
      ...
      withProgress(message = "Creating pipeline...", value = 0.3, {
        tryCatch({
          ...
          proj$set_learner(learner)

          showNotification("Pipeline created successfully!", type = "message")
        }, error = function(e) {
