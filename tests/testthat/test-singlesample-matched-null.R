...
# Source helper: pROC is required by singlesample_matched_null_benchmark.
...
test_that("singlesample_matched_null_benchmark: signal panel beats null at p < 0.05 (headline behaviour)", {
  X <- .make_X()
  dat <- .make_y_and_inject(X, .true_panel)
  scoring_fn <- function(M) rowSums(log(M + 1))
  res <- singlesample_matched_null_benchmark(dat$X, dat$y,
                                        panel = .true_panel,
...
test_that("singlesample_matched_null_benchmark: fails when panel not in colnames(X)", {
  X <- .make_X()
  y <- stats::rbinom(nrow(X), 1L, 0.5)
  expect_error(
    singlesample_matched_null_benchmark(X, y, panel = "nonexistent_mir",
                                   scoring_fn = function(M) rowSums(M), K = 10L),
    "None of the panel features"
  )
})

test_that("singlesample_matched_null_benchmark: null panels do NOT overlap observed panel", {
  X <- .make_X()
  dat <- .make_y_and_inject(X, .true_panel)
  scoring_fn <- function(M) rowSums(log(M + 1))
  res <- singlesample_matched_null_benchmark(dat$X, dat$y,
...
test_that("singlesample_matched_null_benchmark: hemolysis markers excluded when post_hemolysis_corrected=TRUE", {
  X <- .make_X()
  set.seed(99L)
  ...
  y <- stats::rbinom(nrow(X), 1L, 0.5)
  scoring_fn <- function(M) rowSums(log(M + 1))
  res <- singlesample_matched_null_benchmark(X, y, panel = .true_panel,
                                        scoring_fn = scoring_fn,
                                        K = 60L, seed = 42L,
...
test_that("singlesample_matched_null_benchmark: tier-3 fallback returns eligible=FALSE, p_emp=NA", {
  set.seed(11L)
  n_s <- 50L; p_s <- 30L
  ...
  res_d <- singlesample_matched_null_benchmark(X_s, y_s, panel = panel_big,
...
test_that("singlesample_bh_fdr_correct_matched_null: returns vector of correct length", {
  fake <- list(list(p_emp = 0.01), list(p_emp = 0.04), list(p_emp = 0.50))
  q <- singlesample_bh_fdr_correct_matched_null(fake)
  expect_length(q, 3L)
  expect_true(all(q >= 0 & q <= 1))
})

test_that("singlesample_bh_fdr_correct_matched_null: BH is monotone relative to p-value order", {
...
test_that("singlesample_holm_correct_familywise: Holm-corrected p values >= raw p values", {
  pvals <- c(0.001, 0.01, 0.04, 0.20)
  adj <- singlesample_holm_correct_familywise(pvals)
...
test_that("singlesample_bh_fdr_correct_blocked: Toray cluster collapses to same block p-value", {
  fake <- list(
    list(p_emp = 0.005),
    list(p_emp = 0.012),
    list(p_emp = 0.04),
  ...
  out <- singlesample_bh_fdr_correct_blocked(fake, blocks)
...
test_that("singlesample_bh_fdr_correct_blocked: q_block_BH >= p_block (BH can only increase)", {
...
test_that("singlesample_bh_fdr_correct_blocked: length mismatch raises error", {
  fake <- list(list(p_emp = 0.01))
  expect_error(singlesample_bh_fdr_correct_blocked(fake, c("A", "B")))
})

test_that("singlesample_bh_fdr_correct_blocked: returns both conservative and textbook columns", {
...
test_that("singlesample_bh_fdr_correct_blocked: textbook reference <= conservative reference (k>=2)", {
  fake <- list(list(p_emp = 0.005), list(p_emp = 0.012),
               list(p_emp = 0.04), list(p_emp = 0.30))
  blocks <- c("Toray", "Toray", "Affy-S", "Agilent-S")
  out <- singlesample_bh_fdr_correct_blocked(fake, blocks)
...
test_that("singlesample_bh_fdr_correct_blocked: k=1 singleton block reproduces cohort p", {
...
test_that("singlesample_bh_fdr_correct_blocked: rho argument propagates", {
  fake <- list(list(p_emp = 0.005), list(p_emp = 0.012))
  blocks <- c("Toray", "Toray")
  out_low  <- singlesample_bh_fdr_correct_blocked(fake, blocks, rho = 0.10)
  out_high <- singlesample_bh_fdr_correct_blocked(fake, blocks, rho = 1.00)
  expect_false(isTRUE(all.equal(out_low$p_block[1], out_high$p_block[1])))
})

test_that("singlesample_bh_fdr_correct_blocked: rho=0 recovers vanilla Fisher", {
  fake <- list(list(p_emp = 0.01), list(p_emp = 0.05))
  blocks <- c("B", "B")
  out <- singlesample_bh_fdr_correct_blocked(fake, blocks, rho = 0)
...
test_that("singlesample_hanley_mcneil_auc_ci returns a bounded interval and SE", {
  ci <- singlesample_hanley_mcneil_auc_ci(auc = 0.72, n_pos = 50, n_neg = 50)
...
test_that("singlesample_hanley_mcneil_auc_ci shrinks with larger sample counts", {
  ci_small <- singlesample_hanley_mcneil_auc_ci(auc = 0.72, n_pos = 25, n_neg = 25)
  ci_large <- singlesample_hanley_mcneil_auc_ci(auc = 0.72, n_pos = 250, n_neg = 250)
  width_small <- ci_small[["ci_hi"]] - ci_small[["ci_lo"]]
  width_large <- ci_large[["ci_hi"]] - ci_large[["ci_lo"]]
  expect_lt(width_large, width_small)
})

test_that("singlesample_matched_null_benchmark: returns AUC CI bracketing auc_obs (hanley_mcneil)", {
  X <- .make_X()
  dat <- .make_y_and_inject(X, .true_panel)
  scoring_fn <- function(M) rowSums(log(M + 1))
  res <- singlesample_matched_null_benchmark(dat$X, dat$y,
...
test_that("singlesample_matched_null_benchmark: auc_ci_method='none' returns NA CI", {
  X <- .make_X()
  dat <- .make_y_and_inject(X, .true_panel)
  scoring_fn <- function(M) rowSums(log(M + 1))
  res <- singlesample_matched_null_benchmark(dat$X, dat$y,
...
test_that("singlesample_matched_null_benchmark: AUC CI shrinks with growing N (hanley_mcneil)", {
  scoring_fn <- function(M) rowSums(log(M + 1))
  ...
  ci_width <- function(n) {
    ...
    res <- singlesample_matched_null_benchmark(X, y, panel = panel,
                                          scoring_fn = scoring_fn,
                                          K = 30L, seed = 11L,
                                          auc_ci_method = "hanley_mcneil")
...
test_that("singlesample_matched_null_benchmark_cv: signal panel passes 5-fold CV", {
  set.seed(99L)
  n_cv <- 150L; p_cv <- 120L
  ...
  res <- singlesample_matched_null_benchmark_cv(
...
test_that("singlesample_matched_null_benchmark_cv: panel selection respects training-fold-only", {
  set.seed(99L)
  n_cv <- 120L; p_cv <- 80L
  X_cv <- matrix(stats::rlnorm(n_cv * p_cv, meanlog = 5, sdlog = 1), nrow = n_cv,
                  dimnames = list(NULL, paste0("f-", seq_len(p_cv))))
...
test_that("singlesample_matched_null_benchmark_cv: grouped folds honour group_id", {
  set.seed(77L)
  ...
  res_grp <- singlesample_matched_null_benchmark_cv(
...
test_that("singlesample_matched_null_benchmark_cv: returns eligible=FALSE under min_valid_folds", {
  set.seed(3L)
  ...
  res_t <- singlesample_matched_null_benchmark_cv(
...
test_that("singlesample_matched_null_benchmark_cv: same seed -> identical p_emp_cv", {
  set.seed(99L)
  n_cv <- 150L; p_cv <- 120L
  X_cv <- matrix(stats::rlnorm(n_cv * p_cv, meanlog = 5, sdlog = 1), nrow = n_cv,
                  dimnames = list(NULL, paste0("f-", seq_len(p_cv))))
  ...
  X_cv[y_cv == 1, paste0("f-", 1:10)] <-
    X_cv[y_cv == 1, paste0("f-", 1:10)] * 3
  res1 <- singlesample_matched_null_benchmark_cv(
    X_cv, y_cv, panel_size = 10L,
    scoring_fn = function(M) rowSums(log(M + 0.5)),
    K = 30L, outer_k = 3L, seed = 17L, min_valid_folds = 2L)
  res2 <- singlesample_matched_null_benchmark_cv(
    X_cv, y_cv, panel_size = 10L,
    scoring_fn = function(M) rowSums(log(M + 0.5)),
    K = 30L, outer_k = 3L, seed = 17L, min_valid_folds = 2L)
...
test_that("singlesample_matched_null_benchmark_cv: returns AUC CI brackets auc_obs_cv", {
  set.seed(99L)
  n_cv <- 150L; p_cv <- 120L
  X_cv <- matrix(stats::rlnorm(n_cv * p_cv, meanlog = 5, sdlog = 1), nrow = n_cv,
                  dimnames = list(NULL, paste0("f-", seq_len(p_cv))))
