test_that("Paper 3 method bank records exported package functions", {
  ...
  expect_silent(paper3_assert_method_bank_exports(bank))
})

.paper3_family_fixture <- function(seed = 37L, n = 96L, p_extra = 52L) {
  set.seed(seed)
  base_features <- c(
...
test_that("learned, Group-DRO, Sinkhorn, and external-validation helpers work", {
  fx <- .paper3_family_fixture(n = 72L, p_extra = 36L)
  ...
  loco <- paper3_make_loco_splits(fx$meta, min_train_cohorts = 1L)
  ...
  locto <- paper3_make_locto_splits(fx$meta, min_train_cohorts = 1L)
