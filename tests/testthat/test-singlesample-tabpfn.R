...
# fit_tabpfn / score_tabpfn / .tabpfn_model_digest and all package helpers are
# already loaded. TabPFN inference needs the Python `tabpfn` module via
# reticulate; tests that fit/score are skipped when tabpfn is unavailable.
skip_if_no_tabpfn <- function() {
  ...
  # interactive browser login, and no network telemetry under the test harness.
...
# discriminative signal TabPFN learns in-context. Abundances are exp() so the
# input is strictly positive (compositional).
# n defaults small (40) and the tests fit/score on device = "auto" (the H100 GPU
# here; falls back to CPU elsewhere) because the row-by-row TabPFN forward pass is
# far too slow on CPU for the suite. The DECISIVE single-sample properties
# (single-row == batch, determinism, row-equivariance) are bit-EXACT on GPU
# (identical inputs -> identical forward); only per-sample scale-invariance carries
# ~2e-6 cuBLAS float noise on GPU, so that one test uses a 1e-5 tolerance. The
# scorer's own DEFAULT device stays "cpu" (exact / reproducible deployment); these
# tests opt into the GPU purely for speed. TabPFN is designed for small in-context
# data, so a 40-row context with a strong planted shift is ample.
.make_tabpfn_data <- function(n = 40L, p = 20L, k = 6L, shift = 1.2,
                              sd = 0.5, seed = 11L) {
  set.seed(seed)
...
.auc_mw_tabpfn <- function(y, score) {
  case <- score[y == 1L]
  ctrl <- score[y == 0L]
...
# A small, fast CPU fit reused across tests.
.fit_small_tabpfn <- function(seed = 11L) {
  d <- .make_tabpfn_data(seed = seed)
  list(model = fit_tabpfn(d$X, d$y, hp = list(device = "auto")), data = d)
}


test_that("fit returns a well-formed tabpfn_model", {
  skip_if_no_tabpfn()
  f <- .fit_small_tabpfn()
  m <- f$model
  expect_s3_class(m, "tabpfn_model")
  expect_equal(m$feature_universe, colnames(f$data$X))
  expect_true(m$case_col %in% c(1L, 2L))
  expect_equal(m$classes, c(0L, 1L))
  expect_equal(m$model_version, "v2")
...
test_that("score is finite, length-correct, and in [0, 1]", {
  skip_if_no_tabpfn()
  f <- .fit_small_tabpfn()
  s <- score_tabpfn(f$model, f$data$X)
...
test_that("larger score = more case-like on a held-out planted shift", {
  skip_if_no_tabpfn()
  d <- .make_tabpfn_data(seed = 11L)          # n = 40 (controls 1:20, cases 21:40)
  tr <- c(1:15, 21:35)
  te <- setdiff(seq_len(nrow(d$X)), tr)       # 5 held-out controls + 5 cases
  m <- fit_tabpfn(d$X[tr, ], d$y[tr], hp = list(device = "auto"))
  s <- score_tabpfn(m, d$X[te, ])
  auc <- .auc_mw_tabpfn(d$y[te], s)
  expect_gt(auc, 0.5)
...
test_that("DECISIVE: single-row score == batch score (conditional single-sample)", {
  ...
  batch <- score_tabpfn(m, X)
  single <- vapply(seq_len(nrow(X)), function(i) {
    score_tabpfn(m, X[i, , drop = FALSE])
  }, numeric(1L))
...
test_that("score is invariant to row permutation", {
  ...
  m <- f$model
  X <- f$data$X[seq_len(8L), , drop = FALSE]
  base <- score_tabpfn(m, X)
  set.seed(7)
  perm <- sample(nrow(X))
  sp <- score_tabpfn(m, X[perm, , drop = FALSE])
...
test_that("score is invariant to per-sample positive scaling", {
  ...
  Xs <- X * scal
  ss <- score_tabpfn(m, Xs)
  # rCLR is exactly scale-invariant; the residual is GPU cuBLAS float noise
  # (~2e-6 on cuda, 0 on cpu), so the tolerance is 1e-5 (still far tighter than
  # any score-meaningful difference).
...
test_that("passes the canonical row-equivariance gate", {
  ...
  expect_invisible(
    singlesample_assert_row_equivariant(
      score_tabpfn, f$model, f$data$X[seq_len(8L), , drop = FALSE],
      model_digest = .tabpfn_model_digest
    )
  )
})
...
test_that("determinism: re-score and refit reproduce identical scores", {
  skip_if_no_tabpfn()
  d <- .make_tabpfn_data(seed = 11L)
  m1 <- fit_tabpfn(d$X, d$y, hp = list(device = "auto", seed = 42L))
  X <- d$X[seq_len(6L), , drop = FALSE]
  s1a <- score_tabpfn(m1, X)
  s1b <- score_tabpfn(m1, X)
  expect_identical(s1a, s1b)                       # re-score identical

  m2 <- fit_tabpfn(d$X, d$y, hp = list(device = "auto", seed = 42L))
  s2 <- score_tabpfn(m2, X)
  ...
  expect_identical(.tabpfn_model_digest(m1), .tabpfn_model_digest(m2))
})
...
test_that("scoring does not mutate the global torch / R RNG state", {
  ...
  # Scoring must also leave the CUDA RNG byte-unchanged on a GPU (same guard
  # wrapper as fit); no-op assertion on CPU-only hosts.
  if (cuda_ok) {
...
test_that("canonical dispatch via singlesample_score_call matches direct score", {
  skip_if_no_tabpfn()
  # Only run if the roster registers ai-tabpfn with a canonical score_fn.
  roster <- tryCatch(singlesample_method_roster(), error = function(e) NULL)
  skip_if(is.null(roster) || !("ai-tabpfn" %in% roster$method_id),
          "ai-tabpfn not yet registered in the roster")
  f <- .fit_small_tabpfn()
  X <- f$data$X[seq_len(6L), , drop = FALSE]
  direct <- score_tabpfn(f$model, X)
  via <- singlesample_score_call("ai-tabpfn", f$model, X)
...
test_that("strict hp allow-list rejects unknown / duplicate / unnamed / bad fields", {
  skip_if_no_tabpfn()
  d <- .make_tabpfn_data(seed = 11L)
  expect_error(fit_tabpfn(d$X, d$y, hp = list(bogus = 1)),
               "unknown hp field")
  expect_error(fit_tabpfn(d$X, d$y, hp = list(seed = 1, seed = 2)),
               "duplicate hp field")
  expect_error(fit_tabpfn(d$X, d$y, hp = list(7)),
               "named")
  expect_error(fit_tabpfn(d$X, d$y, hp = list(device = "tpu")),
               "device")
  expect_error(fit_tabpfn(d$X, d$y, hp = list(n_estimators = 0)),
               "n_estimators")
  expect_error(fit_tabpfn(d$X, d$y, hp = list(max_context = 1)),
               "max_context")
  expect_error(fit_tabpfn(d$X, d$y, hp = list(min_features = 2.5)),
               "min_features")
  expect_error(fit_tabpfn(d$X, d$y, hp = list(seed = 1.5)),
               "seed")
})

test_that("context cap produces a stratified, seeded, deterministic subsample", {
  skip_if_no_tabpfn()
  d <- .make_tabpfn_data(n = 120L, seed = 11L)
  m <- fit_tabpfn(d$X, d$y, hp = list(device = "auto", max_context = 40L))
  ...
  m2 <- fit_tabpfn(d$X, d$y, hp = list(device = "auto", max_context = 40L))
...
test_that("context cap keeps both classes under extreme imbalance + tiny cap", {
  # Regression (pure R, no tabpfn needed) for two bugs that dropped a class:
  ...
  # hit base R's sample(x, 1) -> sample(1:x, 1) gotcha and picked a wrong row. The
  # fix reserves >= 1 slot per present class (cap >= 2) and samples POSITIONS via
  # sample.int(length(idx), k).
  sub <- .tabpfn_context_subsample
...
test_that("fit leaves the CUDA torch RNG byte-unchanged (GPU only)", {
  ...
  invisible(fit_tabpfn(d$X, d$y, hp = list(device = "cuda")))
