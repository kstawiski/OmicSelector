...
test_that("Amendment #4 roster loads with canonical columns and frozen membership", {
  ros <- paper3_method_roster()
  expect_equal(nrow(ros), 72L)
  ...
  expect_setequal(ros$method_id, .paper3_frozen_roster_ids())
...
test_that("roster per-estimand counts match the frozen prespecification (section 4.4)", {
  ros <- paper3_method_roster()
  expect_equal(sum(ros$role == "discriminator" & ros$estimand == "within"), 41L)
  ...
  # Headline discriminator bank = 41 within firm + 24 transfer = 65 (+1 conditional).
  expect_equal(sum(ros$is_discriminator & ros$role != "discriminator-conditional"), 65L)
})
...
test_that("paper3_method_bank() carries the Amendment #4 route columns without regressing", {
  bank <- paper3_method_bank()
  expect_true(all(c("method_id", "estimand", "role", "single_sample_gate",
...
test_that("row-equivariance harness PASSES a single-sample-deployable scorer", {
  ...
  expect_invisible(paper3_assert_row_equivariant(score_fun, model, X))
  expect_true(paper3_is_row_equivariant(score_fun, model, X))
})
...
test_that("row-equivariance harness REJECTS a batch-coupled scorer", {
  ...
  expect_error(paper3_assert_row_equivariant(score_fun, model, X),
               regexp = "row-equivariance violated")
  expect_false(paper3_is_row_equivariant(score_fun, model, X))
})
...
test_that("default model_digest refuses external-pointer (torch-like) state", {
  digest <- OmicSelector:::.paper3_default_model_digest
  has_ptr <- OmicSelector:::.paper3_contains_externalptr
  ...
  expect_invisible(paper3_assert_row_equivariant(
...
test_that("row-equivariance harness propagates per-specimen metadata correctly", {
  ...
  expect_invisible(paper3_assert_row_equivariant(score_fun, model, X, meta))
})

test_that("paper3_score_call dispatches a registered adapter", {
  reg <- OmicSelector:::.paper3_score_adapter_registry
  on.exit(if (exists("ws-alr-pivot", envir = reg, inherits = FALSE))
    rm("ws-alr-pivot", envir = reg), add = TRUE)
  paper3_register_score_adapter("ws-alr-pivot", function(model, X, meta) {
    as.numeric(rowSums(X))
  })
  X <- matrix(1:6, nrow = 3)
  expect_equal(paper3_score_call("ws-alr-pivot", model = NULL, X = X),
               as.numeric(rowSums(X)))
})

test_that("paper3_score_call errors clearly for a not-yet-implemented method", {
  expect_error(
    paper3_score_call("lrt-bw", model = NULL, X = matrix(0, 2, 3)),
    regexp = "not yet implemented")
})

test_that("paper3_score_call validates the score output contract by type", {
  reg <- OmicSelector:::.paper3_score_adapter_registry
  on.exit(if (exists("ws-alr-pivot", envir = reg, inherits = FALSE))
    rm("ws-alr-pivot", envir = reg), add = TRUE)
  ...
  paper3_register_score_adapter("ws-alr-pivot",
                                function(model, X, meta) matrix(rowSums(X), ncol = 1))
  out <- paper3_score_call("ws-alr-pivot", model = NULL, X = X)
  expect_null(dim(out))
  ...
  expect_error(paper3_score_call("ws-alr-pivot", model = NULL, X = X),
