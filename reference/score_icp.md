...
invariant-and-predictive logistic learned by
...
selected features, and the frozen logistic linear predictor \$\$\eta =
intercept + \sum\_{j \in S} \beta_j \\ z_j\$\$ is returned (NOT
[`plogis()`](https://rdrr.io/r/stats/Logistic.html) of it); larger is
more case-like. Here \\z_j\\ is the specimen's own rCLR coordinate for
selected feature \\j\\. Because rCLR centres each specimen by its own
geometric mean over present parts, the score is EXACTLY invariant to
...
- model:
  ...
  An `icp_model` object returned by
  [`fit_icp`](https://kstawiski.github.io/OmicSelector/reference/fit_icp.md).
