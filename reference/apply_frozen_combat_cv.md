...
“\`r \# Option 1: Use PipeOp in mlr3pipelines (recommended) po_combat
\<- create_frozen_combat_pipeop(batch_col = "batch") graph \<- po_combat

\# Option 2: Manual application in custom CV loop for (fold in folds)
result \<- apply_frozen_combat_cv( data = features, batch =
batch_vector, train_indices = fold\$train, test_indices = fold\$test )
\# Use result\$corrected_train and result\$corrected_test “\`

\## WRONG: Do NOT do this! “\`r \# WRONG: Applying ComBat to all data
before CV causes leakage! corrected_all \<- sva::ComBat(all_data, batch)
\# LEAKAGE! cv_result \<- run_cv(corrected_all) \# Inflated performance
“\`
