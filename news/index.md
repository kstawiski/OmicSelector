...
- Bound the external `ranktreeEnsemble` adapters’ training-only
  ...
  default on ordinary panels while preventing protection/C-stack failure
  on high-dimensional miRNA inputs. Prediction now reconstructs and
  requires only the exact screened raw-feature subset rather than
...
- Add audited group-collapsed and profile-weighted inference helpers for
  ...
  families, and outcome-independent complete-support panels. The paper
...
- Make the paper’s all-method comparison consume an immutable, fully
  ...
  byte, exact source and installed-package pins, route-specific
  execution receipts, splitter manifests, and the structural witness
  transitively through the candidate manifest. Snapshot construction,
  execution, and validation additionally bind a deterministic digest of
  every file in the loaded OmicSelector installation, preventing a
  same-version but stale lazy-load database from passing a source-script
  check. Fair-inference snapshots export predictions only for
...
- Add a package-owned post-run validator for the fair all-method
  ...
  multiplicity families, inference and resampling contracts, provenance
  identity, and source/snapshot/runtime pins before atomically promoting
  a PASS receipt.
